def tower_of_hanoi(P):
    ...
    Solution for two disks is simple. Let `A` be the original rod, `B` the destination
     and `C` the auxiliary rod. Problem can be solved in following steps.
    ...
    First we need to solve Tower of Hanoi problem for rods `A` and `C` and `h-1` (or
     first three disks), which is recursive step 1. We're going to need to use rod `B`
     as an auxiliary.

    Then we move the last remaining disk from A to B (step 2). After that we solve Tower
     of Hanoi for remaining disks or `h-1-1` for rods `C` and `B`, this time using rod
     `A` as an auxiliary (recursive step 3).
