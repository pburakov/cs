...
factor** :math:`\\alpha`. Load factor guarantees that the unused space never exceeds a
constant fraction of a total allocated space.

In this implementation, we set :math:`\\alpha=1/2` for insert and :math:`\\alpha=1/4` for
delete operations. Elementary insert/delete operations take :math:`O(1)` time. Since the
amortized cost of each operation is bounded by a constant, the actual time for any
sequence of :math:`n` operations on a dynamic table is :math:`O(n)`.

For a basic dynamic table, without a slot-mapping function, no fast lookups are supported.
...
class DynamicTable:
    ...
    def allocate(self, s):
        ...
        replaces the old table with newly allocated version. Allocation is allowed only
        if newly allocated table can hold all current elements not marked as erased.
...
def table_insert(T, x):
    ...
    The element is simply added to an existing array into an allocated slot. Once the
    table has approached its current limit of memory slots, it needs to be expanded. All
    the elements from an old table must be carried over to a new one.

    Complexity:
        :math:`O(1)` amortized. Elementary insertion, which statistically happens most
        of the time, has a constant cost.

    :param DynamicTable T: Dynamic table.
    :param object x: An element to insert.
...
def table_delete(T, i):
    ...
    An elementary delete operation is implemented as a trivial removal of the specified
    item from the table. In order to minimize the amount of wasted space, the table needs
    to be contracted once the "critical mass" of empty slots has been reached. This is
    done by a memory reallocation, analogous to :data:`table_insert()`.
