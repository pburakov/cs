...
This implementation uses an array of size :math:`n` to store the elements and guarantee
:math:`O(1)` time for LIFO operations. :math:`n` is the maximum amount of elements that
the stack can hold. Elements are never physically removed from the array. It's only the
pointer to the top of the stack, represented by an array index, that is updated. This s
pointer keeps elements that are still used in the stack from being replaced by newly added
elements.
