...
operations are possible, let `1` be the equal cost of each:
...
edit distance of string "cat" and "nuts" is 3 (add `s`, replace `a` with `u`, replace `c`
with `n`).

This solution to this problem is used in spell checkers and to quantify the similarity of
DNA sequences. It is described in great detail in the book *Algorithm Design Manual* by S.
Skiena.
"""
...
def backtrack(S, T, i=None, j=None):
    ...
    Please note, that we don't need to perform and string modifications, only compute the
    cost. We can define a recursive algorithm using the observation that the last
    character in the string must either be matched, substituted, inserted or deleted. If
    we knew the costs associated with these edits we could decide which option leads to
    the best solution.

    This is a top-down approach, although correct, it is impossibly slow, at every
    position in the string the recursion branches three ways. There are several subroutines
    used here to improve the clarity of the code.

    Complexity:
        :math:`O(3^n)`, recursion grows at a rate of at least :math:`3^n`, even faster.
...
def dp(S, T):
    ...
    intermediate transformations of the string :math:`S` (not implemented).

    Complexity:
        :math:`O(nm)`. Use of dynamic programming allows us to upgrade the speed of this
        algorithm to polynomial time, sacrificing :math:`O(mn)` space.
