...
segmented into a space-separated sequence of dictionary words. Words can appear more than
once. Build the resulting sequence.

Example::

    D = {"a", "man", "plan", "panama", "canal"}
    s = "amanaplanacanal"
    break(S, s)=["a", "man", "a", "plan", "a", "canal"]
...
def backtrack(S, D, P, i=0):
    ...
    Recursive approach is straightforward. The string is traversed. If the prefix is found
    in a dictionary, the procedure is repeated for the remainder of the string.

    Complexity:
        :math:`O(n^2 d)` on the upper bound, where :math:`n` is the length of a string and
        :math:`d` is the size of a dictionary. Unfortunately, :math:`O(n^2)` extra space
        is used for prefix storage and memory stack is absorbed on every match.

    :param str S: Input string.
    :param set[str] D: Dictionary of words.
    :param list P: Mutable output list of words.
    :param int i: Starting index (used in recursion).
    :return: :data:`True` if the whole string has been matched.
...
def dp(S, D):
    ...
    state. :math:`DP[0..n-1]` stores matching prefixes found at :math:`i`-th position.
    ...
    Complexity:
        :math:`O(n^2 d)` time, :math:`O(nd)` space.
...
def dfs(DP, P, i):
    ...
    Complexity:
        :math:`O(w)` where :math:`w` is number of matched words.

    :param list DP: List of found and mapped words.
    :param P: Mutable output list of words.
