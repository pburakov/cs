def quicksort(A, p=0, r=None):
    ...
    These left and right sub-arrays are called partitions. They are not immediately sorted,
     and the same operation is then applied recursively to both partitions.
...
def partition(A, p, r):
    ...
    Partitioning procedure rearranges subarray `A[p..r]` in place. As it runs, the
     array becomes partitioned into four (possibly empty) regions. `A[p..i]` contains
     only elements that a less than pivot element `x`. `A[i..j]` contains only elements
